...
async def generate_embeddings(
    ...
    """
    Generate embeddings for text using the Infinity model.
    This allows direct access to the embedding functionality.
    """
    try:
        ...
        # Use the model specified in the request or the default one
        model_name = request.model or settings.INFINITY_EMBEDDINGS_MODEL
        ...
        if isinstance(request.text, list):
            embeddings = embedder.embed_documents(request.text)
            text_count = len(request.text)
        else:
            embeddings = embedder.embed_query(request.text)
            text_count = 1
...
# Endpoint to initiate a conversation for user files
@router.post("/initiate-for-files", response_model=schemas.ConversationInitiateResponse, status_code=status.HTTP_201_CREATED)
async def initiate_for_files(
    current_user: schemas.User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    """
    Creates a new conversation specifically for user-uploaded files.
    Returns the conversation ID.
    """
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Could not initiate conversation for files."
        )
...
@router.get("/debug/collections", response_model=Dict[str, Any])
async def debug_list_collections(
    ...
    try:
        collections = crud.get_all_collections(db, include_inactive=True)
        result = []
        ...
        for col in collections:
            result.append({
                ...
                "is_global_default": col.is_global_default,
                "is_admin_only": col.is_admin_only,
                "created_at": col.created_at.isoformat() if col.created_at else None
            })
        
        # Try to check Milvus collections
        try:
            from pymilvus import connections, utility
            connections.connect(uri=settings.MILVUS_URI)
            milvus_collections = utility.list_collections()
        except Exception as e:
            milvus_collections = f"Error listing Milvus collections: {str(e)}"
        
        # Get the global default collection
        try:
            global_default = crud.get_global_default_collection(db)
            global_default_info = {
                "id": global_default.id,
                "name": global_default.name
            } if global_default else None
        except Exception as e:
            global_default_info = f"Error getting global default: {str(e)}"
        ...
        try:
            ...
            rag_config = f"Error getting RAG config: {str(e)}"
        
        return {
            "collections": result,
            "global_default": global_default_info,
...
@router.post("/debug/create-test-collection", response_model=Dict[str, Any])
async def debug_create_test_collection(
    current_user: schemas.User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    """Debug endpoint to create a test collection with global_default flag set."""
    if current_user.role != models.UserRole.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admin users can access this endpoint"
    ...
    try:
        # First, unset global default on any existing collections
        existing_default = crud.get_global_default_collection(db)
        if existing_default:
            existing_default.is_global_default = False
            db.commit()
            print(f"DEBUG: Unset global_default flag on collection {existing_default.id}: {existing_default.name}")
        
        # Create a new test collection
        collection_name = f"test_global_collection_{int(time.time())}"
        new_collection = models.Collection(
            name=collection_name,
            description="Test collection for global default",
            user_id=current_user.id,
            is_active=True,
            is_admin_only=False,
        ...
        print(f"DEBUG: Created new test collection: {new_collection.id}: {new_collection.name}")
        
        # Create the collection in Milvus as well
        try:
            from app.services.ingestion_service import DocumentIngestionService
            ingestion_service = DocumentIngestionService()
            result = ingestion_service.create_new_collection(collection_name)
            print(f"DEBUG: Created collection in Milvus: {result}")
        except Exception as e:
            print(f"DEBUG: Failed to create collection in Milvus: {str(e)}")
        
        # Update the RAG config to point to this collection
        try:
            from app.services.admin_config_service import AdminConfigService
            AdminConfigService.set_predefined_collection(db, collection_name)
            print(f"DEBUG: Updated predefined_collection in RAG config to: {collection_name}")
        except Exception as e:
            print(f"DEBUG: Failed to update RAG config: {str(e)}")
        ...
        return {
            ...
            "collection": {
                ...
                "is_global_default": new_collection.is_global_default
            }
        }
    except Exception as e:
        import traceback
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error creating test collection: {str(e)}"
        )

@router.post("/debug/reset-conversation-collection/{conversation_id}", response_model=Dict[str, Any])
async def debug_reset_conversation_collection(
    conversation_id: str,
    current_user: schemas.User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    """Debug endpoint to reset and recreate the conversation collection."""
    if current_user.role != models.UserRole.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admin users can access this endpoint"
    ...
    try:
        ...
        try:
            ...
            print(f"DEBUG: Deleted existing collection: {safe_collection_name}")
        except Exception as e:
            print(f"DEBUG: Error deleting collection: {str(e)}")
        
        # Create a new collection
        try:
            success = ingestion_service.create_new_collection(collection_name)
            print(f"DEBUG: Created new collection: {safe_collection_name}, success={success}")
        except Exception as e:
            print(f"DEBUG: Error creating new collection: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        for file in files:
            try:
                file_id = file.id
                print(f"DEBUG: Reprocessing file: {file_id}")
                
                # Reset the file's processing status
                if file.file_metadata:
                    file.file_metadata.pop("is_processed_for_rag", None)
                    file.file_metadata.pop("vector_ids", None)
                    db.commit()
                
                # Process the file
                success = process_file_for_collection_sync(file_id, collection_name, db, conversation.user_id)
                if not success:
                    print(f"DEBUG: Failed to process file: {file_id}")
            except Exception as e:
                print(f"DEBUG: Error processing file {file.id}: {str(e)}")
        
        # Verify the collection
        try:
            # Use admin-configured top_k from RAGConfigService
            top_k = RAGConfigService.get_retriever_top_k(db)
            retriever = vector_store_manager.get_retriever(collection_name, top_k=top_k)
            print(f"DEBUG: Successfully created retriever for collection: {safe_collection_name} with top_k={top_k}")
        except Exception as e:
            print(f"DEBUG: Error creating retriever: {str(e)}")
        
        return {
            "status": "success",
            "message": f"Collection {safe_collection_name} has been reset and files reprocessed",
            "collection_name": safe_collection_name,
            "files_processed": [f.id for f in files]
        }
    except Exception as e:
        import traceback
        traceback.print_exc()
